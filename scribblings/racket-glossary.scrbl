...
Since Racket allows more characters in identifiers than most programming
languages, naming conventions are more varied:
@itemize[
  @item{Name parts are usually separated by hypens (kebap case, e.g.
    @racket[vector-length]). Snake case (@tt{vector_length}) or camel case
    (@tt{vectorLength}) are @italic{not} used. Sometimes module-level
    constants are in all uppercase, e.g. @linebreak[]
    @racket[(define GRID-ROWS 10)].}
  @item{Abbreviations are very rare, almost everything is ``spelled out.'' This
    can result in rather long names. Here are a few extreme cases:@linebreak[]
    @racket[make-input-port/read-to-peek]@linebreak[]
    @racket[write-bytes-avail/enable-break]@linebreak[]
    @racket[call-with-default-reading-parameterization]}
  @item{Predicates end with a question mark, e.g. @racket[string?] or
    @racket[null?]. Comparison predicates often follow the pattern
    @tt{@italic{@tt{type}}=?}, e.g. @racket[string=?].}
  @item{Mutating procedures end with an exclamation point, e.g. @racket[set!]
    or @racket[vector-set!].}
  @item{An asterisk suffix usually means repetition (e.g.
    @racket[regexp-replace*]), nesting (e.g. @racket[let*]) or just a
    variant of the name without asterisk (e.g. @racket[vector*-length]).}
  @item{Class names use a @tt{%} suffix and are @italic{not} capitalized, e.g.
    @racket[frame%].}
  @item{A slash often means ``for'' or ``with,'' e.g. @racket[call/cc] or
    @racket[define/match].}
  @item{Conversion functions are often named after the pattern
    @tt{@italic{@tt{thing}}->@italic{@tt{other-thing}}}, e.g.
    @racket[vector->list].}
  @item{Names with a type name usually have the type name in front, e.g.
    @racket[vector-ref] or @racket[string-copy]. The accessor functions that
    are generated by @racket[struct] definitions also follow this pattern.}
  @item{Functions that create a new instance of a type are often named
    @tt{make-@italic{@tt{struct-name}}}, e.g. @racket[make-base-namespace] or
    @racket[make-channel].}]
